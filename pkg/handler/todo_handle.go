// Define functions (controller) here
...
func GetTodoById(writer http.ResponseWriter, request *http.Request) {
	params := mux.Vars(request) // get values in "request", for example, 
								// we have a route 'r.HandleFunc("/api/todo/{id}", handler).Methods("GET")',
								// then '{id} is extracted and saved to a map named as 'params'

	id, err := strconv.Atoi(params["id"]) // convert 'params["id"]' from string to int
